...
struct CRegistryHelperLoad {
	...
	{
		do_load<boost::is_base_and_derived<IPureLîadableObject<IReader>,Head>::value>(self,p1);
	}
};
...
struct CRegistryHelperSave {
	...
	IC	static void process(T1 *self, T2 p1)
	{
		do_save<boost::is_base_and_derived<IPureSavableObject<IWriter>,Head>::value>(self,p1);
	}
};
