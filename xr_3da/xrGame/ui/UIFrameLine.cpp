...
void CUIFrameLine::Init(LPCSTR base_name, int x, int y, int size, bool horizontal, DWORD align)
{
	...
	string256		buf;
	// frame
	elements[flBack].CreateShader	(strconcat   (sizeof(buf), buf,base_name,"_back"),	"hud\\default");
	elements[flFirst].CreateShader	(strconcat   (sizeof(buf), buf,base_name,"_b"),		"hud\\default");
	elements[flSecond].CreateShader	(strconcat   (sizeof(buf), buf,base_name,"_e"),		"hud\\default");
}
