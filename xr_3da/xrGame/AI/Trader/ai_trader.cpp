...
u8 CAI_Trader::get_anim_count(LPCSTR anim)
{
	...
	for (int i=0; ; ++i) {
		if (0 != smart_cast<CSkeletonAnimated*>(Visual())->ID_Cycle_Safe(strconcat(sizeof(s_temp), s_temp,anim,itoa(i,s,10))))  count++;
		else break;
	}
...
void CAI_Trader::select_head_anim(u32 type)
{
	...
	// construct name
	string128 s1,s2;
	m_tpHeadDef = smart_cast<CSkeletonAnimated*>(Visual())->ID_Cycle_Safe(strconcat(sizeof(s2), s2,*it->second.name,itoa(index,s1,10)));
}
