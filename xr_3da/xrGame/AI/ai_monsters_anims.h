...
public:
	...
	IC	void		Load(CSkeletonAnimated *tpKinematics, LPCSTR caBaseName)
	{
		...
		for (int i=0; ; ++i)
			if (0 != (tpMotionDef = tpKinematics->ID_Cycle_Safe(xr_strconcat(S1,caBaseName,itoa(i,S2,10)))))
				A.push_back(tpMotionDef);
			else
				if (0 != (tpMotionDef = tpKinematics->ID_FX_Safe(xr_strconcat(S1,caBaseName,itoa(i,S2,10)))))
					A.push_back(tpMotionDef);
				else
					break;
	}
...
public:
	...
	IC	void		Load(CSkeletonAnimated *tpKinematics, LPCSTR caBaseName)
	{
		...
		A.resize	(j);
		for (int i=0; i<j; ++i)
			A[i] = tpKinematics->ID_Cycle_Safe(xr_strconcat(S,caBaseName,caBaseNames[i]));
	}
};
