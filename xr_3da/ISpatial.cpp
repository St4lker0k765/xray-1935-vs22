...
void			ISpatial_DB::insert		(ISpatial* S)
{
	...
	if (!bValid)	
	{
		...
		if	(O)			Debug.fatal(DEBUG_INFO, "Invalid OBJECT position or radius (%s)",O->cName());
			...
			if (P)		Debug.fatal(DEBUG_INFO, "Invalid PS spatial position{%3.2f,%3.2f,%3.2f} or radius{%3.2f}",VPUSH(S->spatial.center),S->spatial.radius);
			else		Debug.fatal(DEBUG_INFO, "Invalid OTHER spatial position{%3.2f,%3.2f,%3.2f} or radius{%3.2f}",VPUSH(S->spatial.center),S->spatial.radius);
		}
	}
#endif
